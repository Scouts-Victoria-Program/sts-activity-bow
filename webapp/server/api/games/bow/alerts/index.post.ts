...
import { BowAlertCreateInput, BowAlertData } from "~/server/types/bowAlert";
...
export default defineEventHandler(
  async (event): Promise<ResponseSuccess | ResponseFailure> => {
    const body = await readBody<BowAlertCreateInput>(event);
    ...
    try {
      const bowAlert = await prisma.bowAlert.create({
        data: {
